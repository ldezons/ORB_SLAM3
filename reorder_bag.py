...
def main(args):
    ...
    parser.add_argument(
        "--max-offset",
        nargs=1,
        help="max time offset (sec) to correct.",
        default="60",
        type=float,
    )
    ...
    output = subprocess.Popen(
        ["rosbag", "info", "--yaml", bagfile], stdout=subprocess.PIPE
    ).communicate()[0]
    print(output)
    ...
    with rosbag.Bag(bagfile, "w") as outbag:
        ...
        for topic, msg, t in rosbag.Bag(orig).read_messages():
            ...
            if time.process_time() - last_time > 0.1:
            ...
            if topic == "/tf" and msg.transforms:
                ...
                outbag.write(
                    topic, msg, msg.header.stamp if diff < args.max_offset else t
                )
            else:
